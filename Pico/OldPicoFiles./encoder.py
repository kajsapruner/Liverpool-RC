...
class Virgil:
    ...
    def forward(self, distance, speed=0.5):
        ...
        # Continue until both wheels have moved the target distance
        while abs(self.left_wheel.encoder_count) < target_counts or abs(self.right_wheel.encoder_count) < target_counts:
            # Compare the encoder counts and adjust speed to keep the robot moving straight
            left_count = self.left_wheel.encoder_count
            right_count = self.right_wheel.encoder_count

            # If the left wheel is ahead, reduce its speed
            if left_count > right_count:
                self.right_wheel.PWM_PIN.duty_u16(int(65535 * speed))
                self.left_wheel.PWM_PIN.duty_u16(int(65535 * (speed * 0.95)))  # Slow down the left wheel
            # If the right wheel is ahead, reduce its speed
            elif right_count > left_count:
                self.left_wheel.PWM_PIN.duty_u16(int(65535 * speed))
                self.right_wheel.PWM_PIN.duty_u16(int(65535 * (speed * 0.95)))  # Slow down the right wheel

            sleep(0.01)  # Short delay to avoid hogging the CPU

        # Stop both wheels after reaching the target distance
        self.left_wheel.stop()
        self.right_wheel.stop()
        
    def right(self, distance, speed=0.5):
        # Turn right by moving the left wheel forward and the right wheel backward
        target_counts = self.left_wheel.calculate_encoder_counts(distance)
        self.left_wheel.encoder_count = 0
        self.right_wheel.encoder_count = 0

        self.left_wheel.forward(speed)
        self.right_wheel.backward(speed)

        while abs(self.left_wheel.encoder_count) < target_counts:
            # Compare the encoder counts and adjust speed to keep the robot turning consistently
            left_count = self.left_wheel.encoder_count
            right_count = self.right_wheel.encoder_count
    ...
    def left(self, distance, speed=0.5):
        # Turn left by moving the right wheel forward and the left wheel backward
        target_counts = self.left_wheel.calculate_encoder_counts(distance)
        self.left_wheel.encoder_count = 0
        self.right_wheel.encoder_count = 0
